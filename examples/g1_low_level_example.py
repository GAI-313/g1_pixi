...
if platform.system() == "Darwin":
    ...
    # .pixi is in the workspace root, which is 1 level up from examples/
...
class Custom:
    ...
    def LowCmdWrite(self):
        ...
        # Use actual motor count from state if possible
        num_motor = min(G1_NUM_MOTOR, len(self.low_state.motor_state))
        ...
        if self.time_ < self.duration_ :
            ...
            # Use max wrist index safely
            max_wrist_idx = max(G1JointIndex.LeftWristRoll, G1JointIndex.RightWristRoll)
            if max_wrist_idx < num_motor:
